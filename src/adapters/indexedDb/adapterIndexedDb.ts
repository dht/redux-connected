import { Adapter, ApiRequest, ApiResponse, ILocalDb, Json } from '../../types';
...
import { IndexedDbQueryBuilder, StorageXPath } from './IndexedDbQueryBuilder';
import {
    deleteByPredicate,
    deleteXPath,
    getXPath,
...
export class IndexedDbAdapter implements Adapter {
    constructor(private localDb: ILocalDb) {}
    ...
    ): Promise<IndexedDbResponse> => {
        const { originalAction } = request;
        const { type } = originalAction;
        const nodeName = this.localDb.actionTypeToNodeName(type);

        const data = await this.localDb.getMany(nodeName);

        if (data && data.length) {
            const dataParsed = itemsToObject(data);
            this.localDb.dispatchSetMany(nodeName, dataParsed);
        }
    ...
    parseReturnedData(request: ApiRequest, res: IndexedDbResponse) {
        const { data } = res;
    ...
    fireRequest = (request: ApiRequest): Promise<ApiResponse> => {
        ...
        return new Promise((resolve) => {
            apiMethod(request, response)
                .then((_res: any) => {
                    const res = _res as IndexedDbResponse;
